...
class EvalDataLoader(AbstractDataLoader):
    ...
    """
    def __init__(self, config, dataset, additional_dataset=None,
                 batch_size=1, shuffle=False,test_data=False):
        ...
        if config['eval_short_seq'] and test_data:
            self.eval_u=self.filter_by_user_hist(filter_max=4)
        else:
    ...
    def filter_by_user_hist(self, filter_min=-1,filter_max=4):
        uid_hist_count = self.additional_dataset.df.groupby(self.additional_dataset.uid_field)[self.additional_dataset.iid_field].count().to_dict()
        filtered_list = []
        all_eval_item = self.dataset.df[self.dataset.uid_field].unique()
        for k in uid_hist_count:
            if int(k) in all_eval_item and int(uid_hist_count[k])>filter_min and int(uid_hist_count[k])<filter_max:
                filtered_list.append(int(k))
