...
class FREEDOM(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # modified by lyf, not save adj for simplicity
        # mm_adj_file = os.path.join(dataset_path, 'mm_adj_{}_{}_{}.pt'.format(self.mm_adj_name,self.knn_k, int(10*self.mm_image_weight)))
        ...
        if self.use_ln:
            ...
            if not self.concat_multimodal_input:
        ...
        if self.v_feat is not None:
            ...
            # print(self.mm_adj)
            # _indices = self.mm_adj._indices()
            # _indices1 = _indices[1,:].reshape((-1,i)).cpu().detach().numpy()
        if self.t_feat is not None:
            indices, text_adj = self.get_knn_adj_mat(self.text_embedding.weight.detach())
            self.mm_adj = text_adj
            # _indices = self.mm_adj._indices()
            # _indices2 = _indices[1,:].reshape((-1,i)).cpu().detach().numpy()
        ...
        #     xxx = []
        #     for idx in range(_indices1.shape[0]):
        #         s1 = set(_indices1[idx])
        #         s2 = set(_indices2[idx])
        #         common_elements = s1.intersection(s2)
        #         xxx.append(len(common_elements))
        #     res.append(np.mean(xxx))
        # print(dataset_path,res)
        # exit(1)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.t_feat is not None:
            if self.use_ln:
                pos_text_feats = self.text_trs(self.t_ln(self.text_embedding.weight[pos_items]))
                neg_text_feats = self.text_trs(self.t_ln(self.text_embedding.weight[neg_items]))
                mf_t_loss = self.bpr_loss(ua_embeddings[users],pos_text_feats, neg_text_feats)
            else:
                text_feats = self.text_trs(self.text_embedding.weight)
                mf_t_loss = self.bpr_loss(ua_embeddings[users], text_feats[pos_items], text_feats[neg_items])
        if self.v_feat is not None:
            if self.use_ln:
                pos_image_feats = self.image_trs(self.v_ln(self.image_embedding.weight[pos_items,:]))
                neg_image_feats = self.image_trs(self.v_ln(self.image_embedding.weight[neg_items,:]))
                mf_v_loss = self.bpr_loss(ua_embeddings[users], pos_image_feats, neg_image_feats)
            else:
