...
class MGCN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        image_adj_file = os.path.join(dataset_path, 'image_adj_mgcn_{}_{}_{}.pt'.format(self.mm_adj_name, self.knn_k, self.sparse))
        text_adj_file = os.path.join(dataset_path, 'text_adj_mgcn_{}_{}_{}.pt'.format(self.mm_adj_name, self.knn_k, self.sparse))
    ...
    def forward(self, adj, train=False):
        if self.v_feat is not None:
            if self.use_ln:
                image_feats = self.image_trs(self.v_ln(self.image_embedding.weight))
            else:
                image_feats = self.image_trs(self.image_embedding.weight)
        if self.t_feat is not None:
            if self.use_ln:
                text_feats = self.text_trs(self.t_ln(self.text_embedding.weight))
            else:
