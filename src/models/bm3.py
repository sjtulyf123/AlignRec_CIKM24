...
class BM3(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # self.loss_file = os.path.join(LOGROOT, logfilename+'_loss.txt')
        # with open(self.loss_file,'a') as f:
        #     f.write('n_layers, reg_weight, dropout: '+str(self.n_layers)+', '+str(self.reg_weight)+', '+str(self.dropout)+'\n')
    ...
    def calculate_loss(self, interactions):
        ...
        if self.use_ln:
            if self.t_feat is not None:
                t_feat_online = self.text_trs(self.t_ln(self.text_embedding.weight[items]))
            if self.v_feat is not None:
                v_feat_online = self.image_trs(self.v_ln(self.image_embedding.weight[items]))
        else:
            if self.t_feat is not None:
        ...
        # with open(self.loss_file,'a') as f:
        #     f.write(str(loss_ui.mean().item())+' '+str(loss_iu.mean().item())+' '+str(self.reg_loss(u_online_ori, i_online_ori)[0].item())+' '+str(loss_t.mean().item())+' '+str(loss_v.mean().item())+' '+str(loss_tv.mean().item())+' '+str(loss_vt.mean().item())+'\n')
